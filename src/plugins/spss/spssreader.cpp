...
static aera_context spss_read(int argc, char **argv)
{
    if (argc < 1)
        return 0;

    Spss::File *sf = new Spss::File(argv[0]);

    return reinterpret_cast<aera_context>(sf);
}

static aera_item spss_pull(aera_context c)
{
    return c;
}

static aera_item spss_meta(aera_context c)
{
    return c;
}

static void spss_close(aera_context)
{
    // sf is deleted from close(root)
}
